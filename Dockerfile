# parameters
ARG REPO_NAME="dt-jupyter"
ARG DESCRIPTION="Duckietown HS curriculum "
ARG MAINTAINER="J Duran (jduran@ttic.edu)"
...
# map container port to the host for remote access
EXPOSE 8888
...
CMD ["bash", "-c", "dts --set-version ${DISTRO}"]
CMD ["bash", "-c", "dts version"]
CMD ["bash", "-c", "jupyter lab --ip 0.0.0.0 --no-browser --allow-root --notebook-dir /data --ServerApp.password='' --ServerApp.token='' "]
